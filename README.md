...
df['calls per minute'] = (df[['total day calls','total eve calls','total night calls']
1) Total minutes = total day minutes + total eve minutes + total night minutes + total intlminutes.
...
scaler = StandardScaler() numerical_cols = ['account length','number vmail messages','total day minutes','total df[numerical_cols] = scaler.fit_transform(df[numerical_cols])
## Modeling
### Classification
